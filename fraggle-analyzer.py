...
if "url_processed_units_dict" not in st.session_state:
    st.session_state.url_processed_units_dict = None
if "all_queries_for_analysis" not in st.session_state:
...
if "openai_api_key_to_persist" not in st.session_state:
...
if "selected_embedding_model" not in st.session_state:
    st.session_state.selected_embedding_model = "all-mpnet-base-v2"
if "processing" not in st.session_state:
    st.session_state.processing = False
if "gcp_nlp_configured" not in st.session_state:
    st.session_state.gcp_nlp_configured = False
if "gcp_credentials_info" not in st.session_state:
    st.session_state.gcp_credentials_info = None
if "entity_analysis_results" not in st.session_state:
    st.session_state.entity_analysis_results = None
if "zyte_api_key_to_persist" not in st.session_state:
    st.session_state.zyte_api_key_to_persist = ""
if "zyte_api_configured" not in st.session_state:
    st.session_state.zyte_api_configured = False
if "last_analysis_granularity" not in st.session_state:
    st.session_state.last_analysis_granularity = "Passage-based (HTML Tags)"
if "s_overlap_val" not in st.session_state:
    st.session_state.s_overlap_val = 2
# ### NEW for Fraggle Analyzer ###
if "fraggle_analysis_results" not in st.session_state:
    st.session_state.fraggle_analysis_results = None
if "cached_url_data" not in st.session_state:
    st.session_state.cached_url_data = {}
...
with st.sidebar.expander(
    "OpenAI API", expanded=not st.session_state.get("openai_api_configured", False)
):
    openai_api_key_input = st.text_input(
        "Enter OpenAI API Key:",
        type="password",
        value=st.session_state.get("openai_api_key_to_persist", ""),
    ...
    if st.button("Set & Verify OpenAI Key", disabled=st.session_state.processing):
        if openai_api_key_input:
            try:
                ...
                (
                    st.session_state.openai_api_key_to_persist,
                    st.session_state.openai_api_configured,
                    st.session_state.openai_client,
                ) = (openai_api_key_input, True, test_client)
...
with st.sidebar.expander(
    "Gemini API", expanded=not st.session_state.get("gemini_api_configured", False)
):
    ...
    if st.button("Set & Verify Gemini Key", disabled=st.session_state.processing):
        if gemini_api_key_input:
            try:
                ...
                (
                    st.session_state.gemini_api_key_to_persist,
                    st.session_state.gemini_api_configured,
                ) = (gemini_api_key_input, True)
                st.success("Gemini API Key Configured!")
                st.rerun()
            except Exception as e:
                st.session_state.gemini_api_key_to_persist, st.session_state.gemini_api_configured = (
                    "",
                    False,
                )
...
with st.sidebar.expander(
    ...
    expanded=not st.session_state.get("zyte_api_configured", False),
):
        ...
        if zyte_api_key_input:
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.session_state.zyte_api_configured = False
                    st.error(
                        f"Zyte Key Failed. Status: {response.status_code}. Message: {response.text[:200]}"
                    )
            except Exception as e:
                st.session_state.zyte_api_configured = False
...
with st.sidebar.expander(
    "Google Cloud NLP API", expanded=not st.session_state.gcp_nlp_configured
):
    ...
    if uploaded_gcp_key is not None:
        try:
            ...
            if "project_id" in credentials_info and "private_key" in credentials_info:
                ...
                st.success(
                    f"GCP Key for project '{credentials_info['project_id']}' loaded!"
                )
            else:
...
if st.session_state.get("gcp_nlp_configured"):
    st.sidebar.markdown("✅ Google NLP API: **Configured**")
else:
    st.sidebar.markdown("⚠️ Google NLP API: **Not Configured**")
st.sidebar.markdown(
    f"✅ Zyte API: **{'Configured' if st.session_state.zyte_api_configured else 'Not Configured'}**"
)

if st.session_state.get(
    "openai_api_key_to_persist"
) and not st.session_state.get("openai_client"):
    st.session_state.openai_client = OpenAI(
        api_key=st.session_state.openai_api_key_to_persist
    )
...
def get_gemini_embeddings(texts: list, model: str):
    ...
    try:
        result = genai.embed_content(
            model=model, content=texts, task_type="RETRIEVAL_DOCUMENT"
        )
...
def get_embeddings(texts, local_model_instance=None):
    ...
    if model_choice.startswith("openai-"):
        return get_openai_embeddings(
            texts, client=st.session_state.openai_client, model=model_choice.replace("openai-", "")
        )
        ...
        if local_model_instance is None:
            ...
            return np.array([])
        return local_model_instance.encode(
            list(texts) if isinstance(texts, tuple) else texts
        )
...
def fetch_content_with_zyte(url, api_key):
    ...
    try:
        ...
        st.error(
            f"Zyte API HTTP Error for {url}: {e.response.status_code} - {e.response.text[:200]}"
        )
        return None
    except Exception as e:
...
def extract_structural_passages_with_full_text(html_content):
    ...
    while i < len(content_elements):
        current_element, current_text = (
            content_elements[i],
            content_elements[i].get_text(separator=" ", strip=True),
        )
        if current_element.name in ["h1", "h2", "h3", "h4", "h5", "h6"] and (
            i + 1
            ...
            i += 1
    final_passages = [
        clean_text_for_display(p) for p in merged_passages if p and len(p.split()) > 2
    ]
...
def add_sentence_overlap_to_passages(structural_passages, overlap_count=2):
    ...
    for i in range(num_passages):
        ...
        prefix = (
            _get_n_sentences(structural_passages[i - 1], overlap_count, from_start=False)
            if i > 0
            else ""
        )
        suffix = (
            _get_n_sentences(structural_passages[i + 1], overlap_count, from_start=True)
            if i < num_passages - 1
            else ""
        )
...
def render_safe_highlighted_html(html_content, unit_scores_map):
    ...
    for element in soup.find_all(target_tags):
        ...
        for unit_text, score in unit_scores_map.items():
            if clean_text_for_display(element_text) == clean_text_for_display(unit_text):
                passage_score = score
                break
            elif clean_text_for_display(unit_text) in clean_text_for_display(element_text):
                passage_score = score
                break
        ...
        if element.name in ["ul", "ol"]:
            list_items_html = "".join(
                [f"<li>{clean_text_for_display(li.get_text())}</li>" for li in element.find_all("li")]
            )
            html_parts.append(
                f"<{element.name} style='{style}'>{list_items_html}</{element.name}>"
            )
        else:
...
def get_sentence_highlighted_html_flat(page_text_content, unit_scores_map):
    ...
    for sentence in sentences:
        ...
        elif len(cleaned_sentence) > 100 and cleaned_sentence[:100] in unit_scores_map:
            sentence_score = unit_scores_map[cleaned_sentence[:100]]
        else:
            found_unit_score = 0.5
            for unit_text, score in unit_scores_map.items():
                if clean_text_for_display(unit_text) in cleaned_sentence:
                    found_unit_score = score
                    break
            sentence_score = found_unit_score

        color = "green" if sentence_score >= 0.70 else "red" if sentence_score < 0.50 else "black"
        highlighted_html += (
            f'<p style="color:{color}; margin-bottom: 2px;">{cleaned_sentence}</p>'
        )
    ...
    try:
        ...
        try:
            for pf in ["```python", "```json", "```"]:
                if content_text.startswith(pf):
                    content_text = content_text.split(pf, 1)[1].rsplit("```", 1)[0].strip()
                    break
            queries = (
                ast.literal_eval(content_text)
            ...
            if not isinstance(queries, list) or not all(
                isinstance(qs, str) for qs in queries
            ):
                raise ValueError("Not list of str.")
            ...
            extracted = [
                ...
                for l in content_text.split("\n")
                if l.strip()
            ]
            if extracted:
                st.warning("Fallback parsing used.")
                return extracted
            return []
    except Exception as e:
...
def render_fraggle_analysis_results(results):
    """Helper function to display all results for the Fraggle Analyzer."""
    st.subheader("📊 Fraggle Analysis Results")
    st.markdown("---")

    # Display summary tables
    st.subheader("Processed Fraggles & Relevance to Main Query")
    st.dataframe(pd.DataFrame(results["fraggle_metrics"]), use_container_width=True)

    st.subheader("Fraggle Coverage by Your Content (Max Similarity)")
    st.dataframe(results["df_fraggle_coverage"], use_container_width=True)
    st.markdown("---")

    # Display detailed analysis with highlighting
    st.subheader("🔬 Detailed Fraggle Coverage & Content Highlighting")
    current_granularity = st.session_state.get(
        "last_analysis_granularity", "Passage-based (HTML Tags)"
    )
    unit_label_type = "Sentence" if current_granularity.startswith("Sentence") else "Passage"

    for i, fraggle_text in enumerate(
        [f["Fraggle Text"] for f in results["fraggle_metrics"]]
    ):
        fraggle_embedding = results["fraggle_embeddings_raw"][i]
        with st.expander(
            f"Fraggle #{i+1}: {fraggle_text[:100]}{'...' if len(fraggle_text) > 100 else ''}",
            expanded=(i == 0),
        ):
            st.write(
                f"**Similarity to Main Query:** {results['fraggle_metrics'][i]['Similarity to Main Query']}"
            )
            st.markdown("---")
            for my_url, data in results["my_url_processed_data"].items():
                st.markdown(f"**Your URL:** [{my_url}]({my_url})")
                if data["unit_embeddings"].size > 0:
                    sims = cosine_similarity(
                        fraggle_embedding.reshape(1, -1), data["unit_embeddings"]
                    ).flatten()
                    max_sim_idx = np.argmax(sims)
                    max_sim_score = sims[max_sim_idx]
                    most_similar_unit = data["units_for_display"][max_sim_idx]

                    st.write(f"**Max Similarity Score:** {max_sim_score:.3f}")
                    st.write(f"**Most Similar {unit_label_type}:**")
                    st.info(most_similar_unit)

                    if st.checkbox(
                        f"Show highlighted text in this URL for this fraggle?",
                        key=f"highlight_rerun_{i}_{my_url}",
                    ):
                        with st.spinner(f"Highlighting {unit_label_type}s in {my_url}..."):
                            unit_scores_for_highlight = {
                                clean_text_for_display(unit_data): score
                                for unit_data, score in zip(data["units_for_display"], sims)
                            }
                            if current_granularity.startswith("Passage"):
                                highlighted_html = render_safe_highlighted_html(
                                    data.get("raw_html"), unit_scores_for_highlight
                                )
                            else:
                                highlighted_html = get_sentence_highlighted_html_flat(
                                    data["page_text_for_highlight"],
                                    unit_scores_for_highlight,
                                )
                            st.markdown(highlighted_html, unsafe_allow_html=True)
                else:
                    st.warning(f"No content units available for {my_url}.")
                st.markdown("---")


def ai_overview_fraggle_analyzer_page():
    st.header("✨ AI Overview Fraggle Analyzer ✨")
    st.markdown(
        "Analyze snippets (fraggles) from AI Overviews against your own content to identify semantic alignment and gaps."
    )

    st.info(
        """
    **Note:** This tool uses the **Scraping Method**, **Analysis Granularity**,
    and **Embedding Model** settings configured in the sidebar. Please adjust them there as needed before running an analysis.
    """
    )
    if st.button("Clear Cached URL Data"):
        st.session_state.cached_url_data = {}
        st.success("URL cache cleared!")
        st.rerun()

    main_query = st.text_input(
        "Main Search Query (used to evaluate fraggle relevance):",
        "benefits of server-side rendering",
        disabled=st.session_state.processing,
    )
    fraggle_input = st.text_area(
        "Paste AI Overview Fraggles (one per line, or separate multi-line fraggles with '---'):",
        "Server-side rendering (SSR) enhances initial page load times.\n---\nImproved SEO is a key benefit of SSR, as search engine crawlers can easily index content.\n---\nSSR helps with faster content display on slow networks.",
        height=200,
        disabled=st.session_state.processing,
    )
    my_urls_input = st.text_area(
        "Enter Your URLs to analyze against fraggles (one per line):",
    ...
    analyze_disabled = (
        not (st.session_state.gemini_api_configured or st.session_state.openai_api_configured)
        or st.session_state.processing
    )
    analyze_button = st.button(
        "🚀 Analyze Fraggles & My Content", type="primary", disabled=analyze_disabled
    )

    if analyze_button:
        st.session_state.processing = True
        st.session_state.fraggle_analysis_active = True
        st.session_state.fraggle_analysis_results = None

        if not all(
            [main_query.strip(), fraggle_input.strip(), my_urls_input.strip()]
        ):
            st.warning("Please fill in all fields: Main Query, Fraggles, and Your URLs.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            st.stop()

        if "---" in fraggle_input:
            fraggle_list = [f.strip() for f in fraggle_input.split("---") if f.strip()]
        else:
            fraggle_list = [f.strip() for f in fraggle_input.splitlines() if f.strip()]

        my_urls = [url.strip() for url in my_urls_input.split("\n") if url.strip()]

        if not fraggle_list or not my_urls:
            st.warning("No valid fraggles or URLs found after parsing.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            st.stop()
        ...
        if not st.session_state.selected_embedding_model.startswith(("openai-", "gemini-")):
            with st.spinner(
                f"Loading local model '{st.session_state.selected_embedding_model}'..."
            ):
                local_embedding_model_instance = load_local_sentence_transformer_model(
                    st.session_state.selected_embedding_model
                )
            if not local_embedding_model_instance:
                st.error("Failed to load local embedding model.")
                st.session_state.processing = False
                st.session_state.fraggle_analysis_active = False
                st.stop()

        try:
            with st.spinner("Embedding main search query..."):
                main_query_embedding = get_embeddings(
                    [main_query], local_embedding_model_instance
                )[0]

            with st.spinner("Embedding all fraggles..."):
                fraggle_embeddings_raw = get_embeddings(
                    fraggle_list, local_embedding_model_instance
                )

            fraggle_metrics = []
            fraggle_query_similarities = cosine_similarity(
                fraggle_embeddings_raw, main_query_embedding.reshape(1, -1)
            ).flatten()
            for i, fraggle_text in enumerate(fraggle_list):
                fraggle_metrics.append(
                    {
                        "Fraggle Text": fraggle_text,
                        "Similarity to Main Query": f"{fraggle_query_similarities[i]:.3f}",
                    }
                )

            my_url_processed_data, urls_to_process = {}, []
            for url in my_urls:
                if url in st.session_state.cached_url_data:
                    st.info(f"✅ Using cached data for: {url}")
                    my_url_processed_data[url] = st.session_state.cached_url_data[url]
                else:
                    urls_to_process.append(url)

            if st.session_state.get("scraping_method", "").startswith("Selenium") and not st.session_state.selenium_driver_instance:
                with st.spinner("Initializing Selenium WebDriver..."):
                    st.session_state.selenium_driver_instance = initialize_selenium_driver()

            for url in urls_to_process:
                st.write(f"⚙️ Fetching and processing: {url}")
                content, method = None, st.session_state.get(
                    "scraping_method", "Requests (lightweight)"
                )
                if method.startswith("Zyte"):
                    content = fetch_content_with_zyte(
                        url, st.session_state.zyte_api_key_to_persist
                    )
                elif method.startswith("Selenium"):
                    content = fetch_content_with_selenium(
                        url, st.session_state.selenium_driver_instance
                    )
                else:
                ...
                s_overlap_val = st.session_state.get("s_overlap_val", 2)

                if current_granularity.startswith("Sentence"):
                    _, page_text_for_highlight = extract_structural_passages_with_full_text(
                        content
                    )
                    ...
                    ) = extract_structural_passages_with_full_text(content)
                    units_for_embedding = add_sentence_overlap_to_passages(
                        units_for_display, s_overlap_val
                    )
                    if not units_for_display and page_text_for_highlight:
                        units_for_display = [page_text_for_highlight]

                if not units_for_embedding:
                    st.warning(f"No processable content units found for {url}. Skipping.")
                    continue

                unit_embeddings = get_embeddings(
                    units_for_embedding, local_embedding_model_instance
                )
                if unit_embeddings.size == 0:
                    st.warning(f"Failed to generate unit embeddings for {url}. Skipping.")
                    continue

                processed_data = {
                    "units_for_display": units_for_display,
                    "units_for_embedding": units_for_embedding,
                    "unit_embeddings": unit_embeddings,
                    "page_text_for_highlight": page_text_for_highlight,
                    "raw_html": content,
                }
                my_url_processed_data[url] = processed_data
                st.session_state.cached_url_data[url] = processed_data

            if st.session_state.selenium_driver_instance:
                st.session_state.selenium_driver_instance.quit()
                st.session_state.selenium_driver_instance = None

            if not my_url_processed_data:
                st.error("No URLs were successfully processed.")
                st.session_state.processing = False
                st.session_state.fraggle_analysis_active = False
                st.stop()

            fraggle_coverage_results = []
            for i, fraggle_text in enumerate(fraggle_list):
                fraggle_embedding = fraggle_embeddings_raw[i]
                for my_url, data in my_url_processed_data.items():
                    if data["unit_embeddings"].size > 0:
                        sims = cosine_similarity(
                            fraggle_embedding.reshape(1, -1), data["unit_embeddings"]
                        ).flatten()
                        ...
                        max_sim_score, most_similar_unit = (
                            0.0,
                            "No units found to compare.",
                        )
                    fraggle_coverage_results.append(
                        {
                            "Fraggle": fraggle_text,
                            "Your URL": my_url,
                            "Max Sim Score": f"{max_sim_score:.3f}",
                            "Most Similar Content Unit": most_similar_unit,
                        }
                    )
            df_fraggle_coverage = pd.DataFrame(fraggle_coverage_results)

            st.session_state.fraggle_analysis_results = {
                "fraggle_metrics": fraggle_metrics,
                "df_fraggle_coverage": df_fraggle_coverage,
                "my_url_processed_data": my_url_processed_data,
                "fraggle_embeddings_raw": fraggle_embeddings_raw,
            }
    ...
    if st.session_state.get("fraggle_analysis_results"):
        render_fraggle_analysis_results(st.session_state.fraggle_analysis_results)


st.title("✨ AI Semantic Analyzer ✨")

# --- TOOL SELECTION ---
current_tool_selection = st.sidebar.selectbox(
    "Select Tool:",
    ["AI Mode Simulator", "AI Overview Fraggle Analyzer"],
    key="main_tool_selector",
)
st.sidebar.divider()

# --- AI MODE SIMULATOR ---
if current_tool_selection == "AI Mode Simulator":
    st.markdown(
        "Fetch, clean, analyze web content against initial & AI-generated queries. Features advanced text extraction and weighted scoring."
    )
    st.sidebar.header("⚙️ Simulator: Input & Query Config")
    ...
    st.session_state.selected_embedding_model = embedding_model_options[
        selected_embedding_label
    ]
    analysis_granularity = st.sidebar.selectbox(
        "Analysis Granularity:",
        ("Passage-based (HTML Tags)", "Sentence-based"),
        index=0,
        disabled=st.session_state.processing,
    )
    ...
    scraping_method = st.sidebar.selectbox(
        ...
        options=["Zyte API (for tough sites)", "Selenium (for dynamic sites)", "Requests (lightweight)"],
        index=0,
        disabled=st.session_state.processing,
    ...
    input_mode = st.sidebar.radio(
        "Choose Input Mode:",
        ("Fetch from URLs", "Paste Raw Text"),
        disabled=st.session_state.processing,
    )
    initial_query_val = st.sidebar.text_input(
    ...
    if input_mode == "Fetch from URLs":
        ...
        use_trafilatura_opt = st.sidebar.checkbox(
            "Use Trafilatura (main content)",
            value=True,
            help="Attempt to use Trafilatura for primary content extraction.",
            disabled=st.session_state.processing,
        )
        run_entity_gap_analysis = st.sidebar.checkbox(
            ...
            disabled=(
                st.session_state.processing or not st.session_state.gcp_nlp_configured
            ),
        )
    else:
        pasted_content_text = st.sidebar.text_area(
            "Paste content here:", height=200, disabled=st.session_state.processing
        )

    num_sq_val = st.sidebar.slider(
        "Num Synthetic Queries:", 3, 50, 5, disabled=st.session_state.processing
    )
    if analysis_granularity.startswith("Passage"):
        s_overlap_val = st.sidebar.slider(
            "Context Sentence Overlap:",
            0,
            10,
            2,
            help="Contextual overlap for similarity calculation.",
            disabled=st.session_state.processing,
        )
        st.session_state.s_overlap_val = s_overlap_val
    else:
        s_overlap_val = 0

    analyze_disabled = not (
        st.session_state.gemini_api_configured
        or st.session_state.openai_api_configured
    )
    if st.sidebar.button(
        "🚀 Analyze Content",
        key="analyze_button",
        type="primary",
        disabled=st.session_state.processing or analyze_disabled,
    ):
        st.session_state.processing = True
    ...
    if st.session_state.processing and "fraggle_analysis_active" not in st.session_state:
        try:
            jobs = []
            ...
            if input_mode == "Fetch from URLs":
                if not urls_text_area_val:
                    ...
                    st.stop()
                jobs = [
                    {"type": "url", "identifier": url.strip()}
                    for url in urls_text_area_val.split("\n")
                    if url.strip()
                ]
            else:
                st.warning("Entity Gap Analysis is only available for URL mode.")
                st.session_state.run_entity_gap_analysis_flag = False
                jobs.append(
                    {
                        "type": "paste",
                        "identifier": "Pasted Content",
                        "content": pasted_content_text,
                    }
                )

            local_embedding_model_instance = None
            if not st.session_state.selected_embedding_model.startswith(
                ("openai-", "gemini-")
            ):
            ...
            local_all_queries = [f"Initial: {initial_query_val}"] + (
                synthetic_queries or []
            )

            with st.spinner("Embedding all queries..."):
                local_all_query_embs = get_embeddings(
                    local_all_queries, local_embedding_model_instance
                )
            ...
            if input_mode == "Fetch from URLs":
                with st.spinner(f"Fetching content from {len(jobs)} URL(s)..."):
                    for job in jobs:
                        ...
                        if method.startswith("Zyte"):
                            fetched_content[url] = fetch_content_with_zyte(
                                url, st.session_state.zyte_api_key_to_persist
                            )
                        elif method.startswith("Selenium"):
                            fetched_content[url] = fetch_content_with_selenium(
                                url, st.session_state.selenium_driver_instance
                            )
                        else:
            ...
            with st.spinner(f"Processing {len(fetched_content)} content source(s)..."):
                for identifier, content in fetched_content.items():
                    ...
                    if not content or len(content.strip()) < 20:
                        st.warning(f"Insufficient text from {identifier}. Skipping.")
                        continue
                    if analysis_granularity.startswith("Sentence"):
                        ...
                        units_for_display = (
                            split_text_into_sentences(page_text_for_highlight)
                    ...
                    if not units_for_embedding:
                        st.warning(
                            f"No processable content units found for {identifier}. Skipping."
                        )
                        continue
                    ...
                    if unit_embeddings.size > 0:
                        unit_sims_to_initial = cosine_similarity(
                            unit_embeddings, initial_query_embedding.reshape(1, -1)
                        ).flatten()
                        weights = np.maximum(0, unit_sims_to_initial)
                        weighted_overall_emb = np.average(
                            unit_embeddings,
                            axis=0,
                            weights=weights if np.sum(weights) > 1e-6 else None,
                        ).reshape(1, -1)
                        overall_sims = cosine_similarity(
                            weighted_overall_emb, local_all_query_embs
                        )[0]
                        unit_q_sims = cosine_similarity(
                            unit_embeddings, local_all_query_embs
                        )
                        local_processed_units_data[identifier][
                            "unit_similarities"
                        ] = unit_q_sims
                            ...
                            local_all_metrics.append(
                                {
                                    ...
                                    "Max Unit Sim.": np.max(current_q_unit_sims)
                                    if current_q_unit_sims.size > 0
                                    else 0.0,
                                    "Avg. Unit Sim.": np.mean(current_q_unit_sims)
                                    if current_q_unit_sims.size > 0
                                    else 0.0,
            ...
            if local_all_metrics:
                ...
                (
                    st.session_state.all_queries_for_analysis,
                    st.session_state.analysis_done,
                ) = (local_all_queries, True)
                st.session_state.last_analysis_granularity = analysis_granularity

            if st.session_state.get("run_entity_gap_analysis_flag"):
                if not st.session_state.gcp_nlp_configured:
                    st.warning(
                        "Entity Gap Analysis was skipped because Google Cloud NLP is not configured."
                    )
                    ...
                    with st.spinner("Performing Entity Analysis on all URLs..."):
                        for identifier, data in st.session_state.url_processed_units_dict.items():
                            ...
                            if full_text:
                                entities = extract_entities_with_google_nlp(
                                    full_text, st.session_state.gcp_credentials_info
                                )
    ...
    if st.session_state.get("analysis_done"):
        ...
        st.expander("View All Analysed Queries").json(
            [
                q.replace("Initial: ", "(Initial) ")
                for q in st.session_state.all_queries_for_analysis
            ]
        )
        unit_label = (
            "Sentence" if st.session_state.last_analysis_granularity.startswith("Sentence") else "Passage"
        )
        st.markdown("---")
        st.subheader(f"📈 Overall Similarity & {unit_label} Metrics Summary")
        ...
        st.markdown("---")
        st.subheader("📊 Visual: Overall Similarity to All Queries (Weighted)")
        fig_bar = px.bar(
            df_display,
            x="Query",
            ...
            color="Source / URL",
            barmode="group",
            title="Overall Content Similarity to Queries",
            height=max(600, 80 * len(st.session_state.all_queries_for_analysis)),
        )
        fig_bar.update_yaxes(range=[0, 1])
        st.plotly_chart(fig_bar, use_container_width=True)
        st.markdown("---")
        st.subheader(f"🔥 {unit_label} Heatmaps vs. All Queries")
        for item_idx, (
            identifier,
            p_data,
        ) in enumerate(st.session_state.url_processed_units_dict.items()):
            with st.expander(
                f"Heatmap & Details for: {identifier}", expanded=(item_idx == 0)
            ):
                if p_data.get("unit_similarities") is None:
                    st.write(f"No {unit_label.lower()} similarity data.")
                    continue
                unit_sims, units, all_queries = (
                    p_data["unit_similarities"],
                    p_data["units"],
                    st.session_state.all_queries_for_analysis,
                )
                short_queries = [
                    q.replace("Initial: ", "(I) ")[:50] + ("..." if len(q) > 50 else "")
                    for q in all_queries
                ]
                unit_labels = [f"{unit_label[0]}{i+1}" for i in range(len(units))]
                wrapped_units = [
                    textwrap.fill(unit, width=100, replace_whitespace=False).replace(
                        "\n", "<br>"
                    )
                    for unit in units
                ]
                hover_text = [
                    [
                        f"<b>{unit_labels[i]}</b> vs Q:'{all_queries[j][:45]}...'<br>Similarity:{unit_sims[i, j]:.3f}<hr><b>Text:</b><br>{wrapped_units[i]}"
                ...
                fig_heat.update_layout(
                    ...
                    height=max(400, 25 * len(short_queries) + 100),
                ...
                selected_query = st.selectbox(
                    f"Select Query for Details:",
                    options=st.session_state.all_queries_for_analysis,
                    key=f"q_sel_{key_base}",
                )
                if selected_query:
                    query_idx = st.session_state.all_queries_for_analysis.index(
                        selected_query
                    )
                    scored_units = sorted(
                        zip(p_data["units"], unit_sims[:, query_idx]),
                        key=lambda item: item[1],
                        reverse=True,
                    )
                    query_display_name = (
                        selected_query.replace("Initial: ", "(I) ")[:30] + "..."
                    )
                    if st.checkbox(
                        f"Show highlighted text for '{query_display_name}'?",
                        key=f"cb_hl_{key_base}_{query_idx}",
                    ):
                        with st.spinner("Highlighting..."):
                            unit_scores_for_query = {
                                unit_text: score for unit_text, score in scored_units
                            }
                            if st.session_state.last_analysis_granularity.startswith(
                                "Passage"
                            ):
                                highlighted_html = render_safe_highlighted_html(
                                    p_data.get("raw_html"), unit_scores_for_query
                                )
                            else:
                                highlighted_html = get_sentence_highlighted_html_flat(
                            ...
                            st.markdown(highlighted_html, unsafe_allow_html=True)
                    if st.checkbox(
                        f"Show top/bottom {unit_label.lower()}s for '{query_display_name}'?",
                        key=f"cb_tb_{key_base}_{query_idx}",
                    ):
                        n_val = st.slider(
                            "N:", 1, 10, 3, key=f"sl_tb_{key_base}_{query_idx}"
                        )
        ...
        with st.expander("🔎 Entity Gap Analysis", expanded=True):
            ...
            if len(url_options) < 2:
                ...
                if primary_url:
                    ...
                    st.write(
                        f"Found **{len(primary_entities)}** unique entities on `{primary_url}`."
                    )
                        ...
                        if url != primary_url:
                            for entity_key, entity_info in entities_data.items():
                                if entity_key not in competitor_entities_map:
                                    competitor_entities_map[entity_key] = {
                                        "info": entity_info,
                                        "found_on": [],
                                    }
                            ...
                            missing_entities.append(
                                {
                                    ...
                                    "Found On (Competitors)": ", ".join(
                                        [f"`{u}`" for u in data["found_on"]]
                                    ),
                                }
                            )
                    if not missing_entities:
                        st.success(
                            f"✅ **No Gaps Found!** Your primary URL covers all entities found on the competitor pages."
                        )
                    else:
                        st.subheader(
                            f"❗️ Found {len(missing_entities)} entities present in other URLs but MISSING from your page:"
                        )
...
# --- AI OVERVIEW FRAGGLE ANALYZER ---
elif current_tool_selection == "AI Overview Fraggle Analyzer":
    ai_overview_fraggle_analyzer_page()

st.sidebar.divider()
st.sidebar.info("Query Fan-Out Analyzer | v5.24 | Fraggle Analyzer Added")
