...
def create_navigation_menu(logo_data_uri):
    ...
    # Combined CSS for menu and for hiding default Streamlit elements
    st.markdown(
        """
        <style>
    ...
    if logo_data_uri:
        st.markdown(
            f'<div class="logo-container"><img src="data:image/png;base64,{logo_data_uri}" width="350"></div>',
            unsafe_allow_html=True
        )
...
# --- (Rest of the Core Functions Start Here) ---
    ...
    try:
        ...
        for i, entity in enumerate(top_primary):
            if entity['Entity'] in entity_embedding_map:
                ...
                best_passage_idx, best_sim_score = np.argmax(sims), np.max(sims)
                if best_sim_score > similarity_threshold:
                    if best_passage_idx not in connected_passages: connected_passages[best_passage_idx] = {'entities': [], 'query_similarity': 0.0}
        ...
        for i, entity in enumerate(top_primary):
            relationships['primary_entities'].append({'id': f"primary_{i}", 'name': entity['Entity'], 'type': entity.get('Type', 'UNKNOWN'), 'combined_score': entity.get('Combined Score', 0), 'node_type': 'primary'})
        for i, entity in enumerate(top_missing):
             relationships['missing_entities'].append({'id': f"missing_{i}", 'name': entity['Entity'], 'type': entity.get('Type', 'UNKNOWN'), 'combined_score': entity.get('Combined Score', 0), 'node_type': 'missing'})
        for passage_idx, connections in connected_passages.items():
            section_id = f"section_{passage_idx}"
            relationships['content_sections'].append({'id': section_id, 'name': f"Section {passage_idx + 1}", 'text': primary_content_passages[passage_idx], 'node_type': 'section', 'connection_count': len(connections['entities'])})
            for entity_connection in connections['entities']:
                relationships['edges'].append({'source': entity_connection['id'], 'target': section_id, 'weight': entity_connection['score'], 'type': 'entity_to_section'})
            if connections['query_similarity'] > similarity_threshold:
                 relationships['edges'].append({'source': 'query', 'target': section_id, 'weight': connections['query_similarity'], 'type': 'query_to_section'})
        ...
        for i, entity in enumerate(all_entities_with_ids):
            if query_entity_sims[i] > query_threshold:
                relationships['edges'].append({'source': 'query', 'target': entity['id'], 'weight': float(query_entity_sims[i]), 'type': 'query_to_entity'})
...
def create_entity_relationship_graph(relationships, selected_missing_entity=None):
    ...
    try:
        ...
        st.warning(f"Spring layout failed ({e}), falling back to circular."); pos = nx.circular_layout(G)
    edge_x, edge_y = [], []
    for edge in G.edges():
    ...
    for node_id, node_data in G.nodes(data=True):
        ...
        border_color, border_width = 'white', 2
        node_type = node_data.get('node_type')
        if node_type == 'query':
            color, icon, size = 'gold', '🎯', 30; node_text.append(f"{icon}")
        elif node_type == 'primary':
            color, icon, size = 'lightblue', '🔵', 15 + (node_data.get('combined_score', 0) * 15)
            node_text.append(f"{icon} {display_name}"); info += f" (Your Content)<br>Type: {node_data['type']}<br>Score: {node_data.get('combined_score', 0):.3f}"
            ...
            if selected_missing_entity and node_data['name'] == selected_missing_entity:
                color, icon, size = 'red', '🔴', 35; border_color, border_width = 'black', 3
            ...
            color, icon, size = '#C8A2C8', '📄', 15 + (node_data.get('connection_count', 0) * 3)
        ...
        for neighbor in G.neighbors(node_id):
            n_data = G.nodes[neighbor]; weight = G.get_edge_data(node_id, neighbor)['weight']
            n_name, n_type = n_data.get('name', 'Unknown'), n_data.get('node_type')
            n_icon = {'primary': '🔵', 'missing': '🟠', 'section': '📄', 'query': '🎯'}.get(n_type, '❓')
            connections.append(f"{n_icon} {n_name} ({weight:.2f})")
        ...
        node_colors.append(color); node_sizes.append(size); node_info.append(info)
        node_border_colors.append(border_color); node_border_widths.append(border_width)
...
def split_text_into_passages(text, passage_length=200):
    if not text: return []
    words = text.split()
    return [' '.join(words[i:i + passage_length]).strip() for i in range(0, len(words), passage_length) if ' '.join(words[i:i + passage_length]).strip()]
...
def generate_semantic_implementation_analysis(entity_name, primary_content, best_passage, target_query, entity_info):
    ...
    try:
        import google.generativeai as genai
        genai.configure(api_key=st.session_state.gemini_api_key_to_persist); model = genai.GenerativeModel("gemini-1.5-pro")
        ...
        implementation_prompt = f'Based on this content analysis and the target query "{target_query}", provide strategic recommendations for implementing the entity "{entity_name}" into the content.\n\nCONTENT TONE ANALYSIS:\n{tone_analysis}\n\nENTITY TO IMPLEMENT: {entity_name}\n- Type: {entity_info.get("Type", "Unknown")}\n- Query Relevance: {entity_info.get("Query Relevance", 0):.3f}\n- Found on {entity_info.get("Found On", 0)} competitor sites\n\nBEST INSERTION LOCATION (Identified by semantic analysis):\n"{best_passage}"\n\nProvide a strategic analysis with:\n\n## Why\n- Strategic value for the query.\n- SEO/topical authority benefits.\n\n## Where\n- Recommend integration into the suggested location.\n- Propose placement strategy (e.g., new paragraph, list item).\n\n## How\n- Provide 2-3 specific, actionable content addition examples.\n- Ensure suggestions maintain the identified tone.\n- Offer natural integration techniques.\n\nKeep recommendations practical and aligned with the content\'s existing voice.'
...
if st.session_state.entity_analysis_results:
    ...
    if not primary_entities: st.error("No entities found in primary URL.")
        ...
        for key, comp_data in competitor_entities.items():
            ...
            if is_missing:
                q_sim = calculate_entity_query_relevance(name, target_query, st.session_state.embedding_model)
                c_score = (comp_data['data']['salience'] + q_sim) / 2
        ...
        else: st.success("✅ No entity gaps found!")
        ...
        if primary_entity_analysis and missing_entities:
            ...
            st.markdown("""_This graph visualizes your content's semantic architecture..._""") # Truncated for brevity
            col1, col2, col3 = st.columns(3)
            with col1: max_primary = st.slider("Max Primary Entities", 5, 25, st.session_state.graph_config['max_primary'], 5, help="Show top N entities from your content")
            with col2: max_missing = st.slider("Max Missing Entities", 5, 20, st.session_state.graph_config['max_missing'], 5, help="Show top N missing entities from competitors")
            with col3: similarity_threshold = st.slider("Connection Threshold", 0.3, 0.7, st.session_state.graph_config['threshold'], 0.05, help="Minimum similarity to show connections")
            ...
            selected_types = st.multiselect("Show entity types:", options=entity_types, default=entity_types, help="Select which entity types to include in the graph")
            min_score = st.slider("Minimum Combined Score", 0.0, 0.8, st.session_state.graph_config['min_score'], 0.05, help="Only show entities above this relevance threshold")
            ...
            with col1: selected_entity_for_graph = st.selectbox("🎯 Select missing entity to highlight in graph:", ["None"] + missing_entity_names, 0, help="Choose a missing entity to make it larger and red for easy identification.")
            with col2:
        ...
        if missing_entities and st.session_state.content_passages.get(primary_url):
            ...
            for entity_info in sorted_missing:
                ...
                entity_options.append(display_name); entity_lookup[display_name] = entity_info
            selected_entity_display = st.selectbox("🔍 Select missing entity to analyze:",[""] + entity_options, 0, help=f"All {len(sorted_missing)} missing entities sorted by Query Relevance.")
            if selected_entity_display and selected_entity_display != "":
                ...
                with col_b:
                    if best_passage_info["similarity"] > 0.1:
                        ...
                        st.text_area("Best passage for adding this entity:", value=best_passage_info["passage"], height=120, disabled=True, key=f"passage_{entity_name.replace(' ', '_')}")
                        ...
                        else: st.info("💡 **Enable Gemini API** in the sidebar to get AI-powered implementation strategies that match your content tone.")
                    else:
...
# Footer
st.sidebar.divider()
st.sidebar.info("🎯 Entity Gap Analysis Tool v2.4")
st.sidebar.markdown("---")
