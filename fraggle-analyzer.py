...
# Store analysis granularity and overlap for the Fraggle Analyzer
if "last_analysis_granularity" not in st.session_state: st.session_state.last_analysis_granularity = "Passage-based (HTML Tags)"
if "s_overlap_val" not in st.session_state: st.session_state.s_overlap_val = 2
...
def render_safe_highlighted_html(html_content, unit_scores_map):
    ...
    for element in soup.find_all(target_tags):
        ...
        for unit_text, score in unit_scores_map.items():
            # Use a more robust check for partial matches or smaller units within larger ones
            # A simple `in` might be too broad if unit_text is very short
            # For this context, it's assumed unit_text is a cleaned passage or sentence.
            # We'll prioritize exact match on cleaned text first.
            if clean_text_for_display(element_text) == clean_text_for_display(unit_text):
                passage_score = score
                break
            # If not an exact match, check if the unit_text is part of the element_text
            # This is less ideal for passage-based where element_text should be *the* unit
            # but for sentences, it's more relevant. We stick to the identified unit itself.
...
def get_sentence_highlighted_html_flat(page_text_content, unit_scores_map):
    ...
    for sentence in sentences:
        ...
        # Directly check if the cleaned sentence is a key in the map
        if cleaned_sentence in unit_scores_map:
            sentence_score = unit_scores_map[cleaned_sentence]
        # Fallback: if cleaned_sentence is a key in the map after trimming
        elif len(cleaned_sentence) > 100 and cleaned_sentence[:100] in unit_scores_map:
             sentence_score = unit_scores_map[cleaned_sentence[:100]] # Use partial match if exact key is very long
        else: # Attempt to find a contained unit for highlighting context
...
st.session_state.last_analysis_granularity = analysis_granularity # Persist for fraggle analyzer
...
if analysis_granularity.startswith("Passage"):
    ...
    st.session_state.s_overlap_val = s_overlap_val # Persist for fraggle analyzer
...
if st.session_state.processing and "fraggle_analysis_active" not in st.session_state: # Only run if not fraggle analysis
    try:
        jobs = []
        ...
        if local_all_metrics:
            ...
            st.session_state.all_queries_for_analysis, st.session_state.analysis_done = True, True # Set analysis_done here
...
# --- NEW TOOL: AI Overview Fraggle Analyzer Page ---
def ai_overview_fraggle_analyzer_page():
    st.header("✨ AI Overview Fraggle Analyzer ✨")
    st.markdown("Analyze snippets (fraggles) from AI Overviews against your own content to identify semantic alignment and gaps.")

    # --- Input Fields ---
    main_query = st.text_input("Main Search Query (used to evaluate fraggle relevance):",
                                "benefits of server-side rendering",
                                disabled=st.session_state.processing)

    fraggle_input = st.text_area("Paste AI Overview Fraggles (one per line, or separate multi-line fraggles with '---'):",
    ...
    my_urls_input = st.text_area("Enter Your URLs to analyze against fraggles (one per line):",
    ...
    analyze_disabled = not (st.session_state.gemini_api_configured or st.session_state.openai_api_configured) or st.session_state.processing
    analyze_button = st.button("🚀 Analyze Fraggles & My Content", type="primary", disabled=analyze_disabled)

    if analyze_button:
        st.session_state.processing = True
        st.session_state.fraggle_analysis_active = True # Flag to prevent other processing blocks from running
        st.session_state.fraggle_analysis_results = None # Reset results for new run
        
        if not main_query.strip():
            st.warning("Please enter a Main Search Query.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            return
        if not fraggle_input.strip():
            st.warning("Please paste at least one fraggle.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            return
        if not my_urls_input.strip():
            st.warning("Please enter at least one URL to compare against.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            return
        ...
        if not fraggle_list:
            st.warning("No valid fraggles found after splitting.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            return
        if not my_urls:
            st.warning("No valid URLs found after splitting.")
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False
            return
        ...
        if not st.session_state.selected_embedding_model.startswith(("openai-", "gemini-")):
            ...
            if not local_embedding_model_instance:
                st.error("Failed to load local embedding model. Please check the model name or your internet connection.")
                st.session_state.processing = False
                st.session_state.fraggle_analysis_active = False
                return
        ...
        try:
            ...
            with st.spinner("Embedding main search query..."):
                main_query_embedding = get_embeddings([main_query], local_embedding_model_instance)[0]

            # 2. Process Fraggles
            st.subheader("Processing Fraggles...")
            fraggle_embeddings_raw = get_embeddings(fraggle_list, local_embedding_model_instance)
            if fraggle_embeddings_raw.size == 0:
                st.error("Failed to generate embeddings for fraggles.")
                st.session_state.processing = False
                st.session_state.fraggle_analysis_active = False
                return

            fraggle_metrics = []
            fraggle_query_similarities = cosine_similarity(fraggle_embeddings_raw, main_query_embedding.reshape(1, -1)).flatten()
            ...
            st.dataframe(pd.DataFrame(fraggle_metrics), use_container_width=True)

            # 3. Process User URLs and their content units
            st.subheader("Processing Your URLs...")
            my_url_processed_data = {}

            # Conditionally initialize Selenium only if it's selected for scraping
            if st.session_state.get("scraping_method", "").startswith("Selenium") and not st.session_state.selenium_driver_instance:
                with st.spinner("Initializing Selenium WebDriver..."): st.session_state.selenium_driver_instance = initialize_selenium_driver()

            for url in my_urls:
                st.info(f"Fetching and processing: {url}")
                content = None
                method = st.session_state.get("scraping_method", "Requests (lightweight)")
                if method.startswith("Zyte"):
                ...
                # Use original app's granularity settings, saved in session state
                current_granularity = st.session_state.get("last_analysis_granularity", "Passage-based (HTML Tags)")
                s_overlap_val = st.session_state.get("s_overlap_val", 2) # Get from session state

                if current_granularity.startswith("Sentence"):
                    _, page_text_for_highlight = extract_structural_passages_with_full_text(content)
                    units_for_display = split_text_into_sentences(page_text_for_highlight) if page_text_for_highlight else []
                    units_for_embedding = units_for_display
                else: # Passage-based
                    units_for_display, page_text_for_highlight = extract_structural_passages_with_full_text(content)
                    units_for_embedding = add_sentence_overlap_to_passages(units_for_display, s_overlap_val)
                    if not units_for_display and page_text_for_highlight: # Fallback if structural passages yield nothing
            ...
            if st.session_state.selenium_driver_instance: # Quit Selenium driver if it was initialized
                st.session_state.selenium_driver_instance.quit()
                st.session_state.selenium_driver_instance = None

            if not my_url_processed_data:
                st.error("No URLs were successfully processed for comparison. Please ensure URLs are accessible.")
                st.session_state.processing = False
                st.session_state.fraggle_analysis_active = False
                return

            # 4. Semantic Comparison: Fraggle-to-My-URL-Passage Similarity
            st.subheader("Fraggle Coverage by Your Content")
            ...
            for i, fraggle_text in enumerate(fraggle_list):
                ...
                for my_url, data in my_url_processed_data.items():
                    if data["unit_embeddings"].size > 0:
                        sims = cosine_similarity(fraggle_embedding.reshape(1, -1), data["unit_embeddings"]).flatten()
                        max_sim_idx = np.argmax(sims)
                        max_sim_score = sims[max_sim_idx]
                        most_similar_unit = data["units_for_display"][max_sim_idx]
                    else:
                        max_sim_score = 0.0
                        most_similar_unit = "No units found to compare."
            ...
            st.dataframe(df_fraggle_coverage, use_container_width=True)

            # 5. Detailed Fraggle Analysis and Highlighting
            st.subheader("Detailed Fraggle Coverage & Content Highlight")
            unit_label_type = "Sentence" if current_granularity.startswith("Sentence") else "Passage"

            for i, fraggle_text in enumerate(fraggle_list):
                fraggle_embedding = fraggle_embeddings_raw[i]
                with st.expander(f"Fraggle: {fraggle_text[:100]}{'...' if len(fraggle_text) > 100 else ''}", expanded=(i==0)):
                    st.write(f"**Similarity to Main Query:** {fraggle_query_similarities[i]:.3f}")
                    st.markdown("---")
                    for my_url, data in my_url_processed_data.items():
                        ...
                        if data["unit_embeddings"].size > 0:
                            ...
                            st.write(f"**Most Similar {unit_label_type}:** {most_similar_unit}")

                            if st.checkbox(f"Show highlighted text in {my_url} for this fraggle?", key=f"highlight_{i}_{my_url}"):
                                with st.spinner(f"Highlighting {unit_label_type}s in {my_url}..."):
                                    # Create a map of text units to their similarity scores for highlighting
                                    ...
                                    if current_granularity.startswith("Passage"):
                                        highlighted_html = render_safe_highlighted_html(data.get("raw_html"), unit_scores_for_highlight)
                                    else:
                                        highlighted_html = get_sentence_highlighted_html_flat(data["page_text_for_highlight"], unit_scores_for_highlight)
                                    st.markdown(highlighted_html, unsafe_allow_html=True)
                        else:
                            st.info(f"No content units available for {my_url}.")
                        st.markdown("---") # Separator between URLs within a fraggle's details
            ...
            st.session_state.processing = False
            st.session_state.fraggle_analysis_active = False # Reset flag
            st.rerun() # Rerun to clear spinner and enable buttons
    
    # Display results if they exist from a previous run
    if st.session_state.get("fraggle_analysis_results"):
        results = st.session_state.fraggle_analysis_results
        st.subheader("Processed Fraggles (from previous run):")
        st.dataframe(pd.DataFrame(results["fraggle_metrics"]), use_container_width=True)
        st.subheader("Fraggle Coverage by Your Content (from previous run):")
        st.dataframe(results["df_fraggle_coverage"], use_container_width=True)

        st.subheader("Detailed Fraggle Coverage & Content Highlight (from previous run)")
        current_granularity = st.session_state.get("last_analysis_granularity", "Passage-based (HTML Tags)")
        unit_label_type = "Sentence" if current_granularity.startswith("Sentence") else "Passage"
        
        for i, fraggle_text in enumerate([f["Fraggle Text"] for f in results["fraggle_metrics"]]):
            fraggle_embedding = results["fraggle_embeddings_raw"][i]
            with st.expander(f"Fraggle: {fraggle_text[:100]}{'...' if len(fraggle_text) > 100 else ''}", expanded=(i==0)):
                st.write(f"**Similarity to Main Query:** {results['fraggle_metrics'][i]['Similarity to Main Query']}")
                st.markdown("---")
                for my_url, data in results["my_url_processed_data"].items():
                    ...
                    if data["unit_embeddings"].size > 0:
                        ...
                        if st.checkbox(f"Show highlighted text in {my_url} for this fraggle?", key=f"highlight_rerun_{i}_{my_url}"):
                            with st.spinner(f"Highlighting {unit_label_type}s in {my_url}..."):
                                unit_scores_for_highlight = {
                                    clean_text_for_display(unit_data): score
                                    for unit_data, score in zip(data["units_for_display"], sims)
...
if st.session_state.get("analysis_done") and st.session_state.all_url_metrics_list and "fraggle_analysis_active" not in st.session_state:
