...
import base64  # For Zyte API decoding
...
if "enhanced_entity_results" not in st.session_state: st.session_state.enhanced_entity_results = None
...
# --- Enhanced Entity Analysis Functions ---
def calculate_entity_query_relevance(entity_name, query_text, local_embedding_model_instance=None):
    """Calculate cosine similarity between entity and query using embeddings."""
    try:
        entity_texts = [
            entity_name,
            f"information about {entity_name}",
            f"{entity_name} details and facts"
        ]
        
        entity_embeddings = get_embeddings(entity_texts, local_embedding_model_instance)
        query_embedding = get_embeddings([query_text], local_embedding_model_instance)
        
        if entity_embeddings.size == 0 or query_embedding.size == 0:
            return 0.0
        ...
        st.warning(f"Failed to calculate entity-query relevance for '{entity_name}': {e}")
        return 0.0

def find_entity_mentions_in_text(entity_name, text_units, unit_embeddings, local_embedding_model_instance=None):
    """Find text units that mention or relate to the entity and return similarity scores."""
    try:
        entity_embedding = get_embeddings([entity_name], local_embedding_model_instance)
        
        if entity_embedding.size == 0 or unit_embeddings.size == 0:
            return []
            
        similarities = cosine_similarity(unit_embeddings, entity_embedding).flatten()
        
        entity_mentions = []
        for i, (sim_score, text_unit) in enumerate(zip(similarities, text_units)):
            if sim_score > 0.3 or entity_name.lower() in text_unit.lower():
                entity_mentions.append({
                    'unit_index': i,
                    'similarity': float(sim_score),
                    'text': text_unit,
                    'contains_entity': entity_name.lower() in text_unit.lower()
                })
        
        entity_mentions.sort(key=lambda x: x['similarity'], reverse=True)
        return entity_mentions
        
    except Exception as e:
        st.warning(f"Failed to find entity mentions for '{entity_name}': {e}")
        return []

def enhanced_entity_gap_analysis(entity_analysis_results, all_queries, url_processed_units_dict, local_embedding_model_instance=None):
    """Enhanced entity analysis that measures salience against queries and finds text locations."""
    enhanced_results = {}
    
    for url, entities_data in entity_analysis_results.items():
        enhanced_results[url] = {
            'entities_by_query': {},
            'entity_locations': {}
        }
        
        url_data = url_processed_units_dict.get(url, {})
        text_units = url_data.get('units', [])
        unit_embeddings = url_data.get('embeddings')
        
        if unit_embeddings is None or len(text_units) == 0:
            continue
        ...
        for query in all_queries:
            ...
            for entity_key, entity_info in entities_data.items():
                query_relevance = calculate_entity_query_relevance(
                    entity_info['name'], 
                    query, 
                    local_embedding_model_instance
                )
                
                entity_mentions = find_entity_mentions_in_text(
                    entity_info['name'],
                    text_units,
                    unit_embeddings,
                    local_embedding_model_instance
                ...
                entity_analysis = {
                    ...
                    'document_salience': entity_info['salience'],
                    'query_relevance': query_relevance,
                    'combined_score': (entity_info['salience'] + query_relevance) / 2,
                    'mentions': entity_mentions[:3],
                    'mention_count': len(entity_mentions)
                }
                
                query_entities.append(entity_analysis)
            
            query_entities.sort(key=lambda x: x['combined_score'], reverse=True)
            enhanced_results[url]['entities_by_query'][query] = query_entities
    
    return enhanced_results

def display_enhanced_entity_analysis(enhanced_results, all_queries, primary_url):
    """Display the enhanced entity gap analysis results."""
    
    st.subheader("🎯 Enhanced Entity Analysis: Query-Specific Relevance")
    
    selected_query = st.selectbox(
        "Select Query for Entity Analysis:",
        options=all_queries,
        key="enhanced_entity_query_selector"
    )
    
    if not selected_query:
        return
        
    st.write(f"**Analyzing entities for query:** `{selected_query}`")
    
    primary_entities = enhanced_results.get(primary_url, {}).get('entities_by_query', {}).get(selected_query, [])
    
    if not primary_entities:
        st.warning(f"No entity data available for primary URL: {primary_url}")
        return
    
    # Display primary URL entity analysis
    with st.expander(f"📊 Primary URL Entities for Selected Query", expanded=True):
        primary_df_data = []
        for entity in primary_entities[:10]:
            primary_df_data.append({
                'Entity': entity['name'],
                'Type': entity['type'],
                'Document Salience': entity['document_salience'],
                'Query Relevance': entity['query_relevance'],
                'Combined Score': entity['combined_score'],
                'Mentions Found': entity['mention_count']
            })
        
        if primary_df_data:
            primary_df = pd.DataFrame(primary_df_data)
            st.dataframe(
                primary_df,
    ...
    # Find gaps
    st.subheader("❗ Entity Gaps for Selected Query")
    
    competitor_entities = {}
    primary_entity_names = {e['name'].lower() for e in primary_entities}
    
    for url, url_data in enhanced_results.items():
        if url == primary_url:
            continue
            
        url_entities = url_data.get('entities_by_query', {}).get(selected_query, [])
        for entity in url_entities:
            entity_key = entity['name'].lower()
            
            is_gap = (entity_key not in primary_entity_names or 
                     any(pe['combined_score'] < 0.4 for pe in primary_entities if pe['name'].lower() == entity_key))
            
            if is_gap and entity['combined_score'] > 0.3:
                if entity_key not in competitor_entities:
                    competitor_entities[entity_key] = {
                        'entity_info': entity,
                        'found_on_urls': []
                    }
                competitor_entities[entity_key]['found_on_urls'].append(url)
    
    if not competitor_entities:
        st.success("✅ No significant entity gaps found for this query!")
    else:
        gap_data = []
        for entity_key, data in competitor_entities.items():
            entity_info = data['entity_info']
            gap_data.append({
                'Missing Entity': entity_info['name'],
                'Type': entity_info['type'],
                'Avg Query Relevance': entity_info['query_relevance'],
                'Avg Combined Score': entity_info['combined_score'],
                'Found on URLs': len(data['found_on_urls']),
                'Competitor URLs': ', '.join([f"`{url}`" for url in data['found_on_urls'][:2]])
            })
        
        gap_df = pd.DataFrame(gap_data)
        gap_df = gap_df.sort_values('Avg Combined Score', ascending=False)
        
        st.dataframe(
            gap_df,
            use_container_width=True,
            column_config={
                "Avg Query Relevance": st.column_config.ProgressColumn(
                    "Query Relevance",
                    format="%.3f",
                    min_value=0,
                    max_value=1,
                ),
                "Avg Combined Score": st.column_config.ProgressColumn(
                    "Combined Score",
                    format="%.3f",
                    min_value=0,
                    max_value=1,
    ...
    # Entity location highlighting
    st.subheader("📍 Entity Locations in Primary URL")
    
    if primary_entities:
        entity_options = [e['name'] for e in primary_entities if e['mention_count'] > 0]
        if entity_options:
            selected_entity_name = st.selectbox(
                "Select entity to highlight in your content:",
                options=entity_options,
                key="entity_location_selector"
            )
            
            if selected_entity_name:
                selected_entity_data = next(
                    (e for e in primary_entities if e['name'] == selected_entity_name), 
                    None
                )
                
                if selected_entity_data and selected_entity_data['mentions']:
                    st.write(f"**Top mentions of '{selected_entity_name}' in your content:**")
                    
                    for i, mention in enumerate(selected_entity_data['mentions']):
                        similarity_color = "🟢" if mention['similarity'] > 0.7 else "🟡" if mention['similarity'] > 0.5 else "🔴"
                        contains_marker = " 🎯" if mention['contains_entity'] else ""
                        
                        st.markdown(f"""
                        **{similarity_color} Mention {i+1}** (Similarity: {mention['similarity']:.3f}){contains_marker}
                        > {mention['text']}
                        """)
                        st.divider()
                else:
                    st.info(f"No mentions found for '{selected_entity_name}' in your content.")
        else:
            st.info("No entities with mentions found in your primary URL content.")
...
def generate_synthetic_queries(user_query, num_queries=7):
    ...
    try:
        ...
        try:
            ...
            if not isinstance(queries,list) or not all(isinstance(qs,str) for qs in queries): 
...
if st.session_state.processing:
    try:
        ...
        st.session_state.enhanced_entity_results = None
        ...
        if st.session_state.get('run_entity_gap_analysis_flag'):
            if not st.session_state.gcp_nlp_configured:
                ...
                # Perform enhanced entity analysis
                if entity_results:
                    with st.spinner("Performing enhanced entity analysis..."):
                        enhanced_results = enhanced_entity_gap_analysis(
                            entity_results,
                            local_all_queries,
                            local_processed_units_data,
                            local_embedding_model_instance
                        )
                        st.session_state.enhanced_entity_results = enhanced_results
...
# --- Enhanced Entity Gap Analysis Display (Moved to the end) ---
if st.session_state.get("enhanced_entity_results"):
    st.markdown("---")
    with st.expander("🔎 Enhanced Entity Gap Analysis", expanded=True):
        results = st.session_state.enhanced_entity_results
        ...
        if len(url_options) < 2:
            ...
            primary_url = st.selectbox(
                ...
                options=url_options,
                index=0,
                key="primary_url_selector_enhanced"
            )

            if primary_url:
                # Display enhanced results
                display_enhanced_entity_analysis(
                    results, 
                    st.session_state.all_queries_for_analysis,
                    primary_url
                )
...
st.sidebar.divider()
st.sidebar.info("Query Fan-Out Analyzer | v6.0 | Enhanced Entity Analysis")
